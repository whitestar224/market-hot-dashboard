...
test("research shows full narrative and server pagination without truncating candidates", () => {
  ...
  assert.match(js, /展开完整 V4\.9 投研、叙事与风控/);
  assert.match(js, /JEV 主判/);
  assert.match(js, /JEV快速主判/);
  assert.match(js, /平均耗时/);
  assert.match(js, /Meta 扩散/);
  assert.match(js, /四条生命线/);
  assert.match(js, /当下市场主线/);
  assert.match(js, /候选与主线关系/);
  assert.match(js, /休眠 \/ 复燃/);
  ...
  assert.match(js, /按专用聊天的完整框架判断、证据与提醒结论展示/);
  assert.match(js, /data-onchain-research-mode="rapid"/);
  assert.match(js, /data-onchain-research-mode="deep"/);
  assert.match(js, /data-onchain-research-mode="hourly"/);
  assert.match(js, /深研实时/);
  assert.match(js, /深研每小时/);
  assert.match(js, /\/api\/onchain-research-mode/);
  assert.match(js, /快速模式不调用聊天深研/);
  assert.match(js, /openedResearch/);
  assert.doesNotMatch(js, /watching\.slice\(0, 12\)/);
  const css = fs.readFileSync("styles.css", "utf8");
  assert.match(css, /\.onchain-research-mode-switch/);
...
test("price watch exposes chain ecosystem mode and API", () => {
  ...
  assert.match(css, /\.onchain-trench-social-tool\.is-person/);
